            ...
            if(isLocationEnabled()){
                fusedLocationProviderClient.lastLocation.addOnCompleteListener(this){task->
                   location=task.result
                    if(location==null && (case<2)){
                        Toast.makeText(this,"Try Again",Toast.LENGTH_SHORT).show()
                        case++
                    }
                    else if(location==null && (case==2)){
                        Toast.makeText(this,"Location is Default",Toast.LENGTH_SHORT).show()
                        location=Location("")
                        location!!.latitude=32.29493609592721
                        location!!.longitude=-9.233985145024436
                        DataConfig.location=location
                        state=true
                    }
                    else{
                        DataConfig.location=location
                        Toast.makeText(this,"Location is Set successfully",Toast.LENGTH_SHORT).show()
                        state=true
                    }
                }
            }
